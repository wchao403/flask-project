...
from inside_frame import constants
...
class User(BaseModel, db.Model):
    ...
    #     # md5 + salt() 如果把常用的密码写在数据库中那么md5就相当于被变相的破解了所以，md5+salt就封装成了 shal sha256
    #     self.password_hash =  generate_password_hash(origin_password)
    @property
    def password(self):
        '''读取属性的方法'''
        # return self.password_hash # 不写这个不行，不能构完整的装饰器，但是又不想要用户读取密码所以可以抛出异常
        raise AttributeError('这个属性不能读取')
